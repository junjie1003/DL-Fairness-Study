...
def validate(val_loader, model):
    ...
    with torch.no_grad():
        for images, labels, _, biases, _, _ in val_loader:
            ...
            logits, _ = model(images)
            logits = sum([F.log_softmax(logits[:, i * num_classes : (i + 1) * num_classes], dim=1) for i in range(num_biases)])
...
def main():
    opt = get_args()
    exp_name = f"di-cifar10s-lr{opt.lr}-bs{opt.batch_size}-epochs{opt.epochs}-seed{opt.seed}"

    # result_dir = f"../results/cifar10s"
    # result_path = Path(result_dir)
    # result_path.mkdir(parents=True, exist_ok=True)
    # fout = open("/".join([str(result_path), "di.txt"]), "w")
    ...
    train_loader = get_cifar10(root, split="train", aug=False, corr=opt.skew_ratio)

    val_loaders = {}
    val_loaders["valid"] = get_cifar10(root, split="valid", aug=False, corr=opt.skew_ratio)
        ...
        if opt.checkpoint:
            ...
            with torch.no_grad():
                ...
                for images, labels, _, biases, _, _ in val_loaders["test"]:
                    ...
                    logits, _ = model(images)
                    logits = sum([F.log_softmax(logits[:, i * num_classes : (i + 1) * num_classes], dim=1) for i in range(num_biases)])
                    preds = logits.data.max(1, keepdim=True)[1].squeeze(1).cpu()
