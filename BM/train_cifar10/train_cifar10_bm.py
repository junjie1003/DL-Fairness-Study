...
def main():
    opt = get_args()
    exp_name = f"bm_{opt.mode}-cifar10s-lr{opt.lr}-bs{opt.batch_size}-epochs{opt.epochs}-seed{opt.seed}"

    # result_dir = f'../results/cifar10s'
    # result_path = Path(result_dir)
    # result_path.mkdir(parents=True, exist_ok=True)
    # fout = open('/'.join([str(result_path), f'bm_{opt.mode}.txt']), 'w')
    ...
    train_loader = get_cifar10(root, split="train", aug=False, under_sample="bin", corr=opt.skew_ratio)
