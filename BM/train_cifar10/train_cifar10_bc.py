...
def main():
    opt = get_args()
    exp_name = f"bc-bb{opt.bb}-cifar10s-lr{opt.lr}-bs{opt.batch_size}-cbs{opt.cbs}-epochs{opt.epochs}-w{opt.weight}-ratio{opt.ratio}-aug{opt.aug}-seed{opt.seed}"

    # result_dir = f'../results/cifar10s'
    # result_path = Path(result_dir)
    # result_path.mkdir(parents=True, exist_ok=True)
    # fout = open('/'.join([str(result_path), 'bc_bb.txt']), 'w')
    ...
    train_loader = get_cifar10(root, batch_size=opt.batch_size, split="train", aug=False, corr=opt.skew_ratio)

    print(f"confusion_matrix - \n original: {train_loader.dataset.confusion_matrix_org}, \n normalized: {train_loader.dataset.confusion_matrix}")

    cont_train_loader = get_cifar10(root, batch_size=opt.cbs, split="train", aug=opt.aug, two_crop=True, ratio=opt.ratio, corr=opt.skew_ratio)
    ...
    val_loaders["test"] = get_cifar10(root, batch_size=256, split="test", aug=False)
