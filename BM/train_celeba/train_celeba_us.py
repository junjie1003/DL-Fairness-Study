...
def main():
    ...
    exp_name = f"us-celeba-lr{opt.lr}-bs{opt.batch_size}-epochs{opt.epochs}-seed{opt.seed}"

    # result_dir = f'../results/celeba'
    # result_path = Path(result_dir)
    # result_path.mkdir(parents=True, exist_ok=True)
    # if opt.target == 'blonde':
    #     fout = open('/'.join([str(result_path), 'us_gender_blond_hair.txt']), 'w')
    ...
    train_loader = get_celeba(root, batch_size=opt.batch_size, target_attr=opt.target, split="train", aug=False, under_sample="ce")
