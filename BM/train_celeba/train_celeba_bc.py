...
def main():
    ...
    exp_name = f"bc-bb{opt.bb}-celeba-lr{opt.lr}-bs{opt.batch_size}-cbs{opt.cbs}-epochs{opt.epochs}-w{opt.weight}-ratio{opt.ratio}-aug{opt.aug}-seed{opt.seed}"

    # result_dir = f"../results/celeba"
    # result_path = Path(result_dir)
    # result_path.mkdir(parents=True, exist_ok=True)
    # if opt.target == "blonde":
    #     fout = open("/".join([str(result_path), "bc_bb_gender_blond_hair.txt"]), "w")
    ...
    cont_train_loader = get_celeba(
        root, batch_size=opt.cbs, target_attr=opt.target, split="train", aug=opt.aug, two_crop=True, ratio=opt.ratio, given_y=True
    )
    ...
    for r in range(repeat_time):
        ...
        if opt.checkpoint:
            ...
            with torch.no_grad():
                ...
                for _, (images, labels, _, biases, _, _) in enumerate(val_loaders["test"]):
                    images = images.cuda()
                    output, _ = model(images)
