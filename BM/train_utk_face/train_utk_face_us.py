...
def main():
    opt = get_args()
    exp_name = f"us-utkface_{opt.sensitive}-lr{opt.lr}-bs{opt.batch_size}-epochs{opt.epochs}-seed{opt.seed}"
    ...
    train_loader = get_utk_face(root, batch_size=opt.batch_size, bias_attr=opt.sensitive, split="train", aug=False, sampling="ce")
