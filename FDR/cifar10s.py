...
# CIFAR-10S dataset
skewed_ratio = 0.95
root = Path(f"{project_dir}/data/cifar10s")
...
def Finetune(model, criterion, train_loader, val_loader, test_loader):
    ...
    # Prepare the dataset
    ################
    x_train, y_train, a_train = prepare_cifar10s_data(train_loader, model)
    print(x_train.shape, y_train.shape, a_train.shape)

    x_test, y_test, a_test = prepare_cifar10s_data(test_loader, model)
    print(x_test.shape, y_test.shape, a_test.shape)

    x_finetune, y_finetune, a_finetune = prepare_cifar10s_data(val_loader, model)
    ...
    optimizer = optim.NAdam(model.out_fc.parameters(), lr=args.lr, weight_decay=5e-4)
...
def main():
    for r in range(repeat_time):
        ...
        ckpt_path = Path(f"{project_dir}/checkpoints/fdr_{args.constraint.lower()}-cifar10s-lr{args.lr}-bs{args.batch_size}-epochs{args.epochs}-seed{args.seed}")
