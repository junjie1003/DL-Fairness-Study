...
def eo_constraint(p, y, a):
    fpr = torch.abs(torch.sum(p * (1 - y) * a) / (torch.sum(a) + 1e-5) - torch.sum(p * (1 - y) * (1 - a)) / (torch.sum(1 - a) + 1e-5))
    fnr = torch.abs(torch.sum((1 - p) * y * a) / (torch.sum(a) + 1e-5) - torch.sum((1 - p) * y * (1 - a)) / (torch.sum(1 - a) + 1e-5))
    return fpr, fnr


def eo_constraint_cifar10s(p, y, a):
    num_classes = 10
    ...
    for class_idx in range(num_classes):
        class_y = (y == class_idx).float()  # 创建一个二进制张量，其中1表示当前类别，0表示其他类别
        class_a = a.float()  # 将a转换为float类型

        # 计算当前类别的FPR和FNR
        fpr = torch.abs(
            torch.sum(p * (1 - class_y) * class_a) / (torch.sum(class_a) + 1e-5)
            - torch.sum(p * (1 - class_y) * (1 - class_a)) / (torch.sum(1 - class_a) + 1e-5)
        )
        fnr = torch.abs(
            torch.sum((1 - p) * class_y * class_a) / (torch.sum(class_a) + 1e-5)
            - torch.sum((1 - p) * class_y * (1 - class_a)) / (torch.sum(1 - class_a) + 1e-5)
        )
    ...
    # 计算FPR和FNR的平均值
    mean_fpr = torch.mean(torch.stack(fpr_list))
    mean_fnr = torch.mean(torch.stack(fnr_list))

    return mean_fpr, mean_fnr


def di_constraint(p, a):
    di = -1 * torch.min(
        (torch.sum(a * p) / torch.sum(a)) / (torch.sum((1 - a) * p) / torch.sum((1 - a))),
...
def mmf_constraint_cifar10s(criterion, log_softmax, y, a):
    # y range from 0 to 9
    # a range from 0 to 1

    loss_combinations = [None] * 20

    for y_val in range(10):
        for a_val in [0, 1]:
            combined_label = y_val * 2 + a_val  # 根据 y 和 a 的取值计算组合标签
            loss_combinations[combined_label] = criterion(log_softmax[(y == y_val) & (a == a_val)], y[(y == y_val) & (a == a_val)])

    # 找到组合中最大的损失，如果没有符合索引的组合，将损失设置为 0
    max_loss = torch.max(torch.stack([loss if loss is not None else torch.tensor(0.0).cuda() for loss in loss_combinations]))
...
def accuracy_equality_difference(y, pred, sensitive_features):
    misclassification_rate_p = sum(y[sensitive_features == 1] != pred[sensitive_features == 1]) / sum(sensitive_features == 1)
    misclassification_rate_n = sum(y[sensitive_features == 0] != pred[sensitive_features == 0]) / sum(sensitive_features == 0)
...
def accuracy_equality_ratio(y, pred, sensitive_features):
    misclassification_rate_p = sum(y[sensitive_features == 1] != pred[sensitive_features == 1]) / sum(sensitive_features == 1)
    misclassification_rate_n = sum(y[sensitive_features == 0] != pred[sensitive_features == 0]) / sum(sensitive_features == 0)
    return min(misclassification_rate_p / (misclassification_rate_n + 1e-6), misclassification_rate_n / (misclassification_rate_p + 1e-6))
...
def print_clf_stats(
    ...
    train_acc, finetune_acc, test_acc = (
        accuracy_score(y_train, pred_train),
        accuracy_score(y_finetune, pred_finetune),
        accuracy_score(y_test, pred_test),
    )
    train_auc, finetune_auc, test_auc = (
        roc_auc_score(y_train, out_train),
        roc_auc_score(y_finetune, out_finetune),
        roc_auc_score(y_test, out_test),
    )
