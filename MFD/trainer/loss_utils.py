...
def compute_feature_loss(inputs, t_inputs, student, teacher):
...
def compute_hinton_loss(outputs, t_outputs=None, teacher=None, t_inputs=None, kd_temp=3):
    ...
    soft_label = F.softmax(t_outputs / kd_temp, dim=1).cuda().detach()
    kd_loss = nn.KLDivLoss(reduction="batchmean")(F.log_softmax(outputs / kd_temp, dim=1), soft_label) * (kd_temp * kd_temp)

    return kd_loss


def compute_at_loss(inputs, t_inputs, student, teacher, for_cifar=False):
