...
class Trainer(hinton_Trainer):
    ...
    def train(self, train_loader, test_loader, epochs):
        ...
        distiller = MMDLoss(w_m=self.lambf, sigma=self.sigma, num_classes=num_classes, num_groups=num_groups, kernel=self.kernel)
        ...
        for epoch in range(self.epochs):
            ...
            print(
                ...
                "Test Loss: {:.3f} Test Acc: {:.2f} Test DEopp {:.2f} [{:.2f} s]".format(
                    epoch + 1, epochs, self.method, eval_loss, eval_acc, eval_deopp, (eval_end_time - eval_start_time)
                )
            )
    ...
    def _train_epoch(self, epoch, train_loader, model, teacher, distiller=None):
        ...
        for i, data in enumerate(train_loader):
            ...
            kd_loss = compute_hinton_loss(stu_logits, t_outputs=tea_logits, kd_temp=self.kd_temp) if self.lambh != 0 else 0
            ...
            if i % self.term == self.term - 1:  # print every self.term mini-batches
                ...
                print(
                    "[{}/{}, {:5d}] Method: {} Train Loss: {:.3f} Train Acc: {:.2f} "
                    "[{:.2f} s/batch]".format(
                        epoch + 1, self.epochs, i + 1, self.method, running_loss / self.term, running_acc / self.term, avg_batch_time / self.term
                    )
                )
            ...
            for c in range(self.num_classes):
                ...
                for g in range(self.num_groups):
                    if len(student[(labels == c) * (groups == g)]) == 0:
                        continue
                    K_TS, _ = self.pdist(
                        teacher[labels == c], student[(labels == c) * (groups == g)], sigma_base=self.sigma, sigma_avg=sigma_avg, kernel=self.kernel
                    )
                    K_SS, _ = self.pdist(
                        student[(labels == c) * (groups == g)],
                        student[(labels == c) * (groups == g)],
                    ...
                    K_TT, _ = self.pdist(teacher[labels == c], teacher[labels == c], sigma_base=self.sigma, sigma_avg=sigma_avg, kernel=self.kernel)
