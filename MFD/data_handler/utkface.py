...
class UTKFaceDataset(VisionDataset):
    ...
    def _delete_others_filter(self):
        races = [self._transform_race(int(image.split('_')[self.fea_map['race']])) for image in self.filename]
        self.num_map['race'] = len(set(races))
        print(self.num_map['race'])
        ages = [self._transform_age(int(image.split('_')[self.fea_map['age']])) for image in self.filename]
        self.num_map['age'] = len(set(ages))
        print(self.num_map['age'])
    ...
    def _filename2SY(self, filename):        
        ...
        if self.sensi == 'race':
            sensi = self._transform_race(sensi)
        if self.label == 'race':
            label = self._transform_race(label)
