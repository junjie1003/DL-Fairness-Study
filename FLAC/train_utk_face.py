...
def main():
    opt = get_args()
    exp_name = f"flac-utkface_{opt.sensitive}-lr{opt.lr}-bs{opt.batch_size}-epochs{opt.epochs}-alpha{opt.alpha}-seed{opt.seed}"

    # result_dir = f"../results/utkface"
    # result_path = Path(result_dir)
    # result_path.mkdir(parents=True, exist_ok=True)
    # if opt.sensitive == "age":
    #     fout = open("/".join([str(result_path), f"flac_age_gender.txt"]), "w")
    # elif opt.sensitive == "race":
    #     fout = open("/".join([str(result_path), f"flac_race_gender.txt"]), "w")
